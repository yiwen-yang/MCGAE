...
for i in range(1, 11):
    ...
    # res = search_res(adata, use_rep="STAGATE", n_clusters=n_clusters, start=0.1, end=2.0, increment=0.04)
    ...
    ari = ari_score(adata.obs['Cell_class'], adata.obs['leiden'])
    print("leiden ari is :", ari)
    import matplotlib.pyplot as plt
    import matplotlib
    matplotlib.use("Agg")
    sc.pl.embedding(adata, basis="spatial3d", projection="3d", color="leiden")
    plt.savefig(os.path.join(file_path, "plot", "STAGATE.pdf"), format="pdf", bbox_inches='tight')


    # Append ARI to CSV
    result_df = pd.DataFrame([[i, "STAGATE", ari]], columns=["Cycle", "method", "ARI"])
