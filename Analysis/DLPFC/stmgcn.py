...
def train_MSpaGCN(opts):
    ...
    for i in range(1, 11):
        ...
        # sc.pl.spatial(features_adata, color=["pred", "Ground Truth"], title=['STMGCN (ARI=%.3f)' % ARI, 'Ground Truth'],
        #               na_in_legend=False, show=True)
    file_path = r"D:\\Work\\MCGAE project\\MCGAE-master\Benchmark\DLPFC\slide151507"

    save_obj.to_csv(os.path.join(file_path, "cluster_output", "DLPFC_Slide151507_STMGCN_ClusterOutput.csv"))
...
def parser_set():
    ...
    parser.add_argument('--sicle', default="111", type=str)
