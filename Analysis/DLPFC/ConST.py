...
for i in range(1, 11):
    ...
    if params.use_pretrained:
        conST_net.load_model('conST_151507.pth')
    else:
    ...
    adata_conST.obs["leiden"]
    save_obj.index = adata.obs.index
    save_obj.index.name = "ID"
    save_obj = pd.concat([save_obj, adata_conST.obs["leiden"]], axis=1)

save_obj.to_csv(os.path.join(file_path, "cluster_output", "DLPFC_Slide151507_ConST_ClusterOutput.csv"))
