...
for n_cells in tqdm(range(1000, 20001, 2000), desc="Processing", position=0, leave=True):
    ...
    x_pixel = subset_adata.obsm["spatial"][:, 0].tolist()
    y_pixel = subset_adata.obsm["spatial"][:, 1].tolist()
    ...
    if l is None:
        l = 6.84
    res = spg.search_res(subset_adata, adj, l, n_clusters, start=0.01, step=0.1, tol=5e-3, lr=0.05, max_epochs=20)
    clf = spg.SpaGCN()
    clf.set_l(l)
    torch.manual_seed(0)
    np.random.seed(0)
    clf.train(subset_adata, adj, init_spa=True, init="louvain", res=res, tol=5e-3, lr=0.05, max_epochs=200)
    y_pred, prob = clf.predict()
    subset_adata.obs["pred"] = y_pred
    subset_adata.obs["pred"] = subset_adata.obs["pred"].astype('category')
...
results_df.to_csv(os.path.join(file_path, "cluster_metric", "SpaGCN_resource_usage.csv"), index=False)
