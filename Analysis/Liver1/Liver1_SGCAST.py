...
class Config(object): # we create a config class to include all paths and parameters 
    def __init__(self):
        ...
        self.spot_paths = [os.path.join(data_path, "filtered_feature_bc_matrix.h5ad")] # in spot_paths, there can be multiple paths and SGCAST will run on the data one by one
...
adata.write(os.path.join(data_path, "filtered_feature_bc_matrix.h5ad"))
...
spots_embeddings = np.loadtxt(os.path.join(data_path,  "filtered_feature_bc_matrix_embeddings.txt"))#
...
sc.pl.spatial(adata, img_key="hires", color='leiden', title="SGCAST")
plt.tight_layout()
plt.savefig(os.path.join(file_path, "result_new", "SGCAST_liver1.pdf"), format="pdf", bbox_inches='tight')
