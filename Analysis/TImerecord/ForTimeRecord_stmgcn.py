...
def train_MSpaGCN(opts):
    ...
    for i in range(1, 2):
        ...
        if opts.dataset == 'DLPFC':
            ...
            test2 = time.time()
            print(f"load data time: {test2-test1}")
        test3 = time.time()    
        adj1, adj2 = load_graph(opts.dataset, opts.sicle, opts.l)
        test4 = time.time()
        print(f"load data time2: {test4-test3}")
        begin = time.time()
        ...
        if opts.initcluster == "kmeans":
            ...
            # print(n_clusters)
            kmeans = KMeans(n_clusters, n_init=7)
        ...
        for epoch in range(opts.max_epochs):
            if epoch % opts.update_interval == 0:
                ...
                # print("y_pred: ", len(np.unique(y_pred)))
                delta_label = np.sum(y_pred != y_pred_last).astype(np.float32) / y_pred.shape[0]
                y_pred_last = y_pred
                # y = labels
                y_pred_last_last = y_pred_last
        ...
        features_adata.obs["pred"] = y_pred_last_last
        ...
        sc.pl.embedding(features_adata, basis="spatial", color="pred", s=20, show=False, title='stmgcn')
        end = time.time()
        print(f"Time cost: {end} seconds")
        print(features_adata.shape[0])
...
def parser_set():
    ...
    parser.add_argument('--dataset', type=str, default='ohter')  # DLPFC
