...
data = sc.read(os.path.join(dir_path, "STARmap_20180505_BY3_1k.h5ad"))
data.var_names_make_unique()
n_clusters = len(np.unique(data.obs["label"]))
data.var_names_make_unique()
...
for i in range(1, 11):
    ...
    st.em.run_pca(data_SME, n_comps=50, random_state=i)
    ...
    from sklearn.metrics import adjusted_rand_score as ari_score
    ari = ari_score(data_SME.obs['label'], data_SME.obs['X_pca_kmeans'])
